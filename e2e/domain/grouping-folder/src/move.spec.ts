...
describe('move', () => {
  ...
  it('should update project references', async () => {
    ...
    expect(
      tsConfig.compilerOptions.paths['@proj/ng-test-app/new-domain/util-layer']
    ).toEqual(['libs/ng-test-app/new-domain/util-layer/src/index.ts']);
    expect(
      tsConfig.compilerOptions.paths[
        '@proj/ng-test-app/new-domain/util-layer/testing'
      ]
    ).toEqual(['libs/ng-test-app/new-domain/util-layer/src/testing.ts']);
  });
});
